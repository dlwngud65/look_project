...
<div id="best">
	...
	<div id="best3" onclick="click5()">
		...
		<div id="answer">
			<span id="answer">비회원 또한 주문이 가능합니다. 구매 하시고자 하는 제품의 색상/사이즈를 선택 후 장바구니
				또는 바로구매클릭 하신다음 주문하기 ...</span>
		</div>
	</div>
	<div id="best1" class="dialog3">
		...
		<div id="answer">
			<span id="answer" class="dialog__content3">비회원 또한 주문이 가능합니다. 구매 하시고자
				하는 제품의 색상/사이즈를 선택 후 장바구니 또는 바로구매클릭 하신다음 주문하기 버튼을 클릭하여 비회원 구매하기로
				주문해주세요 ^^</span>
		</div>
	</div>
</div>
...
if ($status == 'search') {
    if ($select == 'subject') {
        $sql = "select * from qa_write where type='product' and subject like '%$search%' order by num desc";
    } else if ($select == 'content') {
        $sql = "select * from qa_write where type='product' and content like '%$search%' order by num desc";
    } else if ($select == 'name') {
        $sql = "select * from qa_write where type='product' and name like '%$search%' order by num desc";
    } else if ($select == 'id') {
        $sql = "select * from qa_write where type='product' and id like '%$search%' order by num desc";
    }
} else if ($status != 'search' || ! $search) {
    $sql = "select * from qa_write where type='product' order by num desc";
}
...
?>
<form name="search_form" action="./qa_main.php?mode=product&status=search"
	method="post">
	<table id="qa_all">
		<tr>
